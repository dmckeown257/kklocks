...
class KKHomeConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            try:
                await KKHomeApiClient(self.hass, user_input).async_test_connection()
                ...
                await self.async_set_unique_id(
                    f"{user_input[CONF_BASE_URL]}::{user_input[CONF_USERNAME] or 'token'}"
                )
        ...
        return vol.Schema(
            {
                ...
                vol.Required(CONF_TENANT_ID, default=user_input.get(CONF_TENANT_ID, DEFAULT_TENANT_ID)): str,
                ...
                vol.Required(CONF_LOGIN_PATH, default=user_input.get(CONF_LOGIN_PATH, DEFAULT_LOGIN_PATH)): str,
                vol.Required(CONF_DEVICES_PATH, default=user_input.get(CONF_DEVICES_PATH, DEFAULT_DEVICES_PATH)): str,
                vol.Required(
                    CONF_DEVICE_DETAIL_PATH,
                    default=user_input.get(CONF_DEVICE_DETAIL_PATH, DEFAULT_DEVICE_DETAIL_PATH),
                ): str,
                vol.Required(CONF_LOCK_PATH, default=user_input.get(CONF_LOCK_PATH, DEFAULT_LOCK_PATH)): str,
                vol.Required(CONF_STATUS_PATH, default=user_input.get(CONF_STATUS_PATH, DEFAULT_STATUS_PATH)): str,
                vol.Required(CONF_UNLOCK_PATH, default=user_input.get(CONF_UNLOCK_PATH, DEFAULT_UNLOCK_PATH)): str,
                vol.Required(
                    CONF_POLL_INTERVAL,
